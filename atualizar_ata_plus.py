...
def main():
    ...
    tarefas = [
        ...
        {
            "comando": f'set GIT_TERMINAL_PROMPT=0 && git add docs/ .gitignore && git diff --quiet && git diff --staged --quiet || (git commit --no-verify -m "Atualização automática N8N - {datetime.now().strftime("%d/%m/%Y %H:%M")}" && git push)',
